...
if ( ! function_exists('mec_display_price'))
{
  function mec_display_price($original_price=0, $sell_price=0, $currency_icon = '$',$retun_type='1',$currency_position='left',$decimal_point=0,$thousand_comma='0') //$retun_type=1 means price overthrough, $retun_type=2 means purchase price, $retun_type=3 means disount, $retun_type=4 menas discount formatted
  {
    ...
    if($retun_type=='1')
    {
      ...
      {
        $return = "<span class='sell-price'><b>".$currency_left.mec_number_format($sell_price,$decimal_point,$thousand_comma).$currency_right."</b></span> <span class='original-price text-muted text-small' style='text-decoration:line-through'>".$currency_left.mec_number_format($original_price,$decimal_point,$thousand_comma).$currency_right."</span> ";
      }
...
if ( ! function_exists('mec_sticky_footer'))
{
  function mec_sticky_footer($store_data=array(),$subscriber_id='',$current_cart=array(),$ecommerce_config=array())
  {
    ...
    <nav aria-label="breadcrumb" class="w-100 d-print-none" id="sticky-footer">
      <ol class="breadcrumb d-flex justify-content-around align-items-center bg-primary m-0">
