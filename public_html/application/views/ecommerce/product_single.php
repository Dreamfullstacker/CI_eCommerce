  ...
  <div class="row bg-white pb-3 margin_md"> 
    <div class="<?php echo $carousel ? 'col-12 col-sm-12 col-md-6 col-lg-4' : 'col-12';?>">
      <?php if($carousel) : ?>
      <article class="article article-style-c shadow-none m-0">            
    ...
    <div class="col-12 col-sm-12 <?php if($carousel) echo 'col-md-6 col-lg-8';?>">
      <div class="card shadow-none my-3">
        <div class="d-flex justify-content-between align-items-start px-3 py-2">
          ...
          <div class="col-4 p-0 text-right">
            <?php if($product_data['stock_display']=='1') : ?>
              <span class="text-success"><b><?php echo $this->lang->line("In Stock"); ?></b></span>
            <?php endif; ?>
          </div>
        </div>
        ...
        <div class="card-subheader px-3 py-2">
          <p id="calculated_price_basedon_attribute">
            <?php echo mec_display_price($product_data['original_price'],$product_data['sell_price'],$currency_icon,'1',$currency_position,$decimal_point,$thousand_comma);?>
          </p>
        </div>

        <div class="card-body product-attribute px-3 py-2">
                  ...
                  foreach ($attribute_list as $key => $value) 
                  {
                    if(in_array($value["id"], $product_attributes))
                    {
                      ...
                      foreach ($options_array as $key2 => $value2)
                      {
                        ...
                        {
                          $options_print.='
                          <div class="custom-control custom-checkbox mr-1 mb-2">
                            ...
                            <label class="custom-control-label" for="'.$temp_id.'">'.$value2.''.$tempu.'</label>
                          </div>';
                        }
                        else 
                        {
                      ...
                      echo '
                          <h4 class="text-muted">'.$value["attribute_name"].$star.'</h6>
                          <div class="py-2">
                            '.$options_print.'   
                          </div>'; 
                    }
          ...
          <!-- Product Reviews -->
          <h3 class="my-3"><?php echo $this->lang->line("Reviews"); ?></h3>

          <?php if($this->ecommerce_review_comment_exist): ?>
              <?php 
              if(empty($review_list_data)) 
              echo '
                <div class="alert alert-warning text-center" id="nodata">
                  <p>'.$this->lang->line("We could not find any review.").'</p>
                </div>';
              else
              {
          ...
          <!-- Prodcut Description -->
          <h3 class="my-3"><?php echo $this->lang->line("Item Details"); ?></h3>
      ...
      <!-- Action Buttons -->
      <div class="d-flex justify-content-around align-items-center bg-primary m-0" id="sticky-footer-product">
        <button class="btn btn-secondary add_to_cart" data-product-id="<?php echo $product_data['id'];?>" data-attributes="<?php echo $product_data['attribute_ids'];?>" data-action="add" type="button" data-toggle="tooltip" title="<?php echo $this->lang->line('Add to Cart');?>"><?php echo $this->lang->line('Add to Cart');?></button>
        <a href="<?php echo $current_cart_url; ?>" class="btn btn-secondary"><?php echo $this->lang->line("Checkout"); ?></a>
      </div>

      <div class="hero p-2 d-none">
